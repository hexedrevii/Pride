...
bool Pride::Renderer::create(int width, int height)
{
  ...
  this->m_renderer = SDL_CreateTexture(this->m_render, SDL_PIXELFORMAT_RGBA8888,
...
void Pride::Renderer::detach_and_draw()
{
  ...
  float scale = std::min(static_cast<float>(width) / this->m_width,
                         static_cast<float>(height) / this->m_height);

  SDL_FRect dst = SDL_FRect{(width - (this->m_width * scale)) * 0.5f,
                            (height - (this->m_height * scale)) * 0.5f,
                            this->m_width * scale, this->m_height * scale};

  SDL_FRect src = SDL_FRect{0, 0, static_cast<float>(this->m_renderer->w),
                            static_cast<float>(this->m_renderer->h)};
